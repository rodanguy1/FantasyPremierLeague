...
from utils.userDataExtractor import get_user_transfers, get_user_chips, get_user_minus_points
...
async def get_users_transfers_and_chips(fpl, user1, user2):
    ...
    if GAME_WEEK != 17:
        ...
        output += "{}:\n    {}\n    Transfers: ```{}```\n".format(user1.name, await get_user_chips(user1),
                                                                  user1_transfers)
        output += "     Minus points: ```{}```\n\n".format(await get_user_minus_points(user1))
        output += "{}:\n    {}\n    Transfers: ```{}```\n".format(user2.name, await get_user_chips(user2),
                                                                  user2_transfers)
        output += "     Minus points: ```{}```\n\n".format(await get_user_minus_points(user2))
    return output
